"""Bit-exact cocotb checks for the optimized Mitchell implementation."""
...
W = 16
SETTLE_NS = 1
N_RANDOM = int(os.environ.get("MITCHELL_OPT_N_RANDOM", "") or 200_000)
...
class Checker:
    ...
    def finish(self):
        if not self.failures:
            self.dut._log.info("%s: %d pairs checked, all match", self.name, self.checked)
            return
        hidden = max(0, len(self.failures) - MAX_REPORTED_FAILURES)
        a, b, got, exp = self.failures[0]
        msg = f"{self.name}: {len(self.failures)} of {self.checked} mismatched"
        if hidden:
            msg += f" ({hidden} not individually logged)"
        raise AssertionError(msg + f"; first failure a={a} b={b} got={got} expected={exp}")
...
async def drive_signed(dut, name, pairs):
    ...
    for a, b in pairs:
        ...
        await Timer(SETTLE_NS, "ns")
        checker.compare(a, b, read_signed(dut.s_mul), signed_wrap(mitchell, a, b, W))
    checker.finish()
...
async def test_unsigned_corners(dut):
    ...
    await drive_unsigned(dut, "optimized Mitchell unsigned corners", [(a, b) for a in ops for b in ops])


@cocotb.test()
async def test_unsigned_sweep(dut):
    pairs, n_corner = sampled_pairs(W, N_RANDOM)
    dut._log.info("optimized Mitchell unsigned: %d pairs (%d corner + %d random)", len(pairs), n_corner, N_RANDOM)
    await drive_unsigned(dut, "optimized Mitchell unsigned sweep", pairs)


@cocotb.test()
async def test_signed_corners(dut):
    ops = corner_operands_signed()
    await drive_signed(dut, "optimized Mitchell signed corners", [(a, b) for a in ops for b in ops])
...
async def test_signed_sweep(dut):
    ...
    dut._log.info("optimized Mitchell signed: %d pairs (%d corner + %d random)", len(pairs), n_corner, N_RANDOM)
    await drive_signed(dut, "optimized Mitchell signed sweep", pairs)
...
async def test_signed_min_negative(dut):
    ...
    await drive_signed(dut, "optimized Mitchell signed -32768 boundary", pairs)
